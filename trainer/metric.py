...
from data_loader.SQuAD import find_answer_type
...
def evaluate(dataset, predictions):
    ...
    for article in dataset.keys():
        para = dataset[article]
        for qa in para["qa_pairs"]:
            total += 1
            answer_type = find_answer_type(qa)
            if qa['query_id'] not in predictions or not answer_type:
                message = 'Unanswered question ' + qa['query_id'] + \
                            ' will receive score 0.'
                print(message, file=sys.stderr)
                continue
            ground_truths = qa['answer'][answer_type]
...
def evaluate_from_file(dataset_file, prediction_file):
    ...
    with open(dataset_file) as f:
        dataset = json.load(f)
    with open(prediction_file) as g:
...
def convert_tokens(eval_dict, qa_id, ans_type_pred, pp1, pp2, nnum):
    answer_dict = {}
    remapped_dict = {}
    for qid, ans_type, p1, p2, num in zip(qa_id, ans_type_pred, pp1, pp2, nnum):
        ...
        if ans_type == 0:
            ...
            answer_dict[str(qid)] = str(num)
            remapped_dict[uuid] = str(num)
    return answer_dict, remapped_dict

def evaluate_by_dict(eval_dict, answer_dict, eval_log_dir):
    f1 = exact_match = total = 0
    f1_spans = exact_match_spans = total_spans  = 0
    f1_number = exact_match_number = total_number = 0
    eval_log_dict = {}
    for key, value in answer_dict.items():
        ...
        if metric_max_over_ground_truths(exact_match_score, prediction, ground_truths) == 0:
            eval_log_dict[key] = {"context": eval_dict[key]["context"],
                                  "question": eval_dict[key]["question"],
                                  "answers": ground_truths,
                                  "answers_pred": prediction}
            
        if eval_dict[key]['answer_type'] == 0:
            total_spans += 1
            exact_match_spans += metric_max_over_ground_truths(
                exact_match_score, prediction, ground_truths)
            f1_spans += metric_max_over_ground_truths(
                f1_score, prediction, ground_truths)
            
        elif eval_dict[key]['answer_type'] == 1:
            total_number += 1
            exact_match_number += metric_max_over_ground_truths(
                exact_match_score, prediction, ground_truths)
            f1_number += metric_max_over_ground_truths(
                f1_score, prediction, ground_truths)
            
    with open(eval_log_dir, 'w') as f:
        json.dump(eval_log_dict, f, indent=4)

    exact_match = 100.0 * exact_match / total
    f1 = 100.0 * f1 / total
    exact_match_spans = 100.0 * exact_match_spans / total_spans
    f1_spans = 100.0 * f1_spans / total_spans
    exact_match_number = 100.0 * exact_match_number / total_number
    f1_number = 100.0 * f1_number / total_number
    return {'exact_match': exact_match, 'f1': f1,
            'exact_match_spans': exact_match_spans, 'f1_spans': f1_spans,
            'exact_match_number': exact_match_number, 'f1_number': f1_number}
