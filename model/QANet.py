...
class CQAttention(nn.Module):
    ...
    def forward(self, C, Q, Cmask, Qmask):
        ...
        Cmask = Cmask.view(batch_size_c, Lc, 1) # (batch_size, Lc, 1)
        Qmask = Qmask.view(batch_size_c, 1, Lq) # (batch_size, 1, Lq)
        S1 = F.softmax(mask_logits(S, Qmask), dim=2) # (batch_size, Lc, Lq)
        S2 = F.softmax(mask_logits(S, Cmask), dim=1) # (batch_size, Lc, Lq)
        A = torch.bmm(S1, Q) # (batch_size, Lc, d_model)
        B = torch.bmm(torch.bmm(S1, S2.transpose(1, 2)), C) # (batch_size, Lc, d_model)
        return C, A, torch.mul(C, A), torch.mul(C, B)
        # out = torch.cat([C, A, torch.mul(C, A), torch.mul(C, B)], dim=2) # (batch_size, Lc, d_model * 4)
        # return out.transpose(1, 2) # (batch_size, d_model * 4, Lc)
...
class AnswerType(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        WC = torch.empty(1, d_model) # (1, d_model)
        WQ = torch.empty(1, d_model) # (1, d_model)
        nn.init.xavier_uniform_(WC)
        nn.init.xavier_uniform_(WQ)
        self.WC = nn.Parameter(WC) # (1, d_model)
        self.WQ = nn.Parameter(WQ) # (1, d_model)
        # dense layer
        self.FNN = nn.Linear(d_model * 2, 2)
    ...
    def forward(self, C, Q, Cmask, Qmask):
        batch_size, d_model, Lc = C.shape
        batch_size, d_model, Lq = Q.shape
        Cmask = Cmask.view([batch_size, 1, Lc]) # (batch_size, 1, Lc)
        Qmask = Qmask.view([batch_size, 1, Lq]) # (batch_size, 1, Lq)
        alpha_C = torch.matmul(self.WC, C) # (batch_size, 1, Lc)
        alpha_Q = torch.matmul(self.WQ, Q) # (batch_size, 1, Lq)
        alpha_C = F.softmax(mask_logits(alpha_C, Cmask), dim=2) # (batch_size, 1, Lc)
        alpha_Q = F.softmax(mask_logits(alpha_Q, Qmask), dim=2) # (batch_size, 1, Lq)
        h_C = torch.matmul(alpha_C, C.transpose(1, 2)) # (batch_size, 1, d_model)
        h_Q = torch.matmul(alpha_Q, Q.transpose(1, 2)) # (batch_size, 1, d_model)
        h_C = h_C.squeeze() # (batch_size, d_model)
        h_Q = h_Q.squeeze() # (batch_size, d_model)
        h = torch.cat([h_C, h_Q], dim=1) # (batch_size, d_model * 2)
        x = self.FNN(h)
        x = self.relu(x)
        x = self.softmax(x)
        return h_C, h_Q, x
...
class CQToNumber(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        self.FNN = nn.Linear(d_model, 10)
        self.relu = nn.ReLU()
        self.softmax = nn.Softmax(dim=1)
...
class NAQANet(nn.Module):
    def __init__(self, word_mat, char_mat,
        ...
        self.position = Pointer(d_model)
        self.PAD = pad
        self.Lc = c_max_len
        self.Lq = q_max_len
        self.dropout = dropout

        self.answer_type = AnswerType(d_model)
        self.cq_to_number = CQToNumber(d_model)
    ...
    def forward(self, Cwid, Ccid, Qwid, Qcid):
        ...
        CT, A, CA, CB = self.cq_att(Ce, Qe, maskC, maskQ) # (batch_size, Lc, d_model) * 4
        X = torch.cat([CT, A, CA, CB], dim=2) # (batch_size, Lc, d_model * 4)
        X = X.transpose(1, 2) # (batch_size, d_model * 4, Lc)
        ...
        for i, blk in enumerate(self.model_enc_blks):
             M0 = blk(M0, maskC, i*(2+2)+1, 7)
        M3 = M0
        p1, p2 = self.position(M1, M2, M3, maskC)

        h_C, h_Q, ans_type = self.answer_type(A.transpose(1, 2), Q, maskC, maskQ)
        num = self.cq_to_number(h_C)
        return ans_type, p1, p2, num
...
if __name__ == "__main__":
    ...
    if test_NAQANet:
        ...
        # test whole QANet
        num_head = 1
        naqanet = NAQANet(wv_tensor, cv_tensor,
                      c_max_len, q_max_len, d_model, train_cemb=False, num_head=num_head)
        p1, p2 = naqanet(context_wids, context_cids,
