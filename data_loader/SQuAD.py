...
def filter_func(config, example):
    filter = (len(example["context_tokens"]) > config.para_limit or
              len(example["ques_tokens"]) > config.ques_limit)
    if example["y1s"] and example["y1s"]:
        filter = filter or (example["y2s"][0] - example["y1s"][0]) > config.ans_limit
    return filter

def find_answer_type(qa):
    answers = qa["answer"]
    len_answer = 0
    for answer_type in answers.keys():
        if answer_type in ('number', 'spans'):
            if answers[answer_type]:
                return answer_type
        # else:
        #     if answers[answer_type] and answers[answer_type]['year']:
        #         return answer_type
    return None
...
def get_examples(filename, data_type, word_counter, char_counter,
    ...
    with open(filename, 'r', encoding='utf8') as fh:
        ...
        for article in tqdm(source.keys()):
            para = source[article]
            context = para['passage'].replace(
                        "''", '" ').replace("``", '" ')
            context_tokens = word_tokenize(context)
            context_chars = [list(token) for token in context_tokens]
            spans = convert_idx(context, context_tokens)
            for token in context_tokens:
                word_counter[token] += len(para["qa_pairs"])
                for char in token:
                    char_counter[char] += len(para["qa_pairs"])
            for qa in para["qa_pairs"]:
                meta["num_q"] += 1
                ...
                for token in ques_tokens:
                    ...
                    for char in token:
                        char_counter[char] += 1
                y1s, y2s, nums = [], [], []
                answer_text_list_in_context = []
                answer_texts = []
                answer_type_category = None
                answers = qa["answer"]
                answerable = 1
                meta["num_q_answerable"] += answerable

                answer_type = find_answer_type(qa)
                if not answer_type:
                    meta['num_q_noanswer'] += 1
                    continue
                elif answer_type == 'spans':
                    answer_type_category = 0
                    answer_text_list = answers[answer_type]
                    answer_text_list_in_context = [answer for answer in answer_text_list if context.find(answer) != -1]
                    if not answer_text_list_in_context:
                        meta['num_q_noanswer'] += 1
                        continue
                    else:
                        # answer_start_list = {answer: context.find(answer) for answer in answer_text_list_in_context}
                        # answer_start = min(answer_start_list.values())
                        # answer_end = max(answer_start_list.values())
                        # last_answer = max(answer_start_list, key=answer_start_list.get)
                        # answer_end += len(last_answer)
                        # answer_texts.append(' '.join(answer_text_list_in_context))
                        for answer in answer_text_list_in_context:
                    ...
                    if answers[answer_type].startswith('-') or not answers[answer_type].isdigit():
                        meta['num_q_noanswer'] += 1
                        continue
                    elif int(answers[answer_type]) > 9:
                        meta['num_q_noanswer'] += 1
                        continue
                    else:
                        answer_type_category = 1
                        num = answers[answer_type]
                        nums.append(num)
                        answer_texts.append(num)
                        meta["num_qa_answerable"] += answerable
                else:
                    meta["num_q_noanswer"] += 1
                    continue
                example = {
                    ...
                    "answer_type": answer_type_category,
                    # "answer_start_list": answer_start_list,
...
def build_features(config, examples, meta, data_type,
    ...
    for example in tqdm(examples):
        ...
        start, end, num = -1, -1, -1
        if example["y1s"] and example["y2s"]:
            start, end = example["y1s"][-1], example["y2s"][-1]
            y1[start], y2[end] = 1.0, 1.0
        else:
            num = int(example["nums"][-1])
...
class SQuAD(Dataset):
    ...
    def __getitem__(self, idx):
        return (self.examples[idx]["context_wids"],
                ...
                self.examples[idx]["num"],
                self.examples[idx]["y1s"],
                self.examples[idx]["y2s"],
                self.examples[idx]["nums"],
                self.examples[idx]["id"],
                self.examples[idx]["answer_type"],
                self.examples[idx]["answerable"])

def collate(data):
    Cwid, Ccid, Qwid, Qcid, y1, y2, num, y1s, y2s, nums, id, answer_type, answerable = zip(*data)
    ...
    num = torch.from_numpy(np.array(num)).long()
    id = torch.from_numpy(np.array(id)).long()
    answer_type = torch.from_numpy(np.array(answer_type)).long()
    answerable = torch.tensor(answerable).long()
    return Cwid, Ccid, Qwid, Qcid, y1, y2, num, y1s, y2s, nums, id, answer_type, answerable
